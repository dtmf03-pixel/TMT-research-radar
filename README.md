# 📡 Audit Research Radar — 엔터·미디어·게임·IT 회계 이슈 모니터링

> IT·미디어·엔터 산업 감사를 준비하며 만든 **회계 이슈 자동 리서치** 리포지토리입니다.
> DART 공시 · 뉴스 · 회계법인 산업 리포트를 매주 자동 수집하고, 업종(엔터/미디어/게임/IT)과
> 회계 토픽(IFRS 15/16·무형자산·플랫폼)으로 분류해 정리합니다.
>
> **대상 기업 67개사** — 삼정KPMG 연결재무제표 감사실적(사업보고서) 중 IT/미디어/엔터/게임/콘텐츠
> 관련 피감사회사 39곳(`kpmg: true`)에, 산업 커버리지 보강을 위한 규모 있는 IT·미디어·통신·엔터사
> 28곳을 더했습니다. 업종은 **엔터·미디어·통신·게임·IT** 5분류.
> ([config/companies.yml](config/companies.yml))

## 왜 만들었나
엔터·게임·플랫폼은 **수익인식 시점(IFRS 15)**, **콘텐츠·개발비 무형자산 인식과 손상(IAS 38)**,
**데이터센터·오피스 리스(IFRS 16)** 등 회계처리 판단이 특히 어려운 산업입니다.
실무 감사에서 반복적으로 쟁점이 되는 이 이슈들을 **공시 1차 근거 + 뉴스 + 회계법인 해석**을
한자리에서 추적하도록 자동화했습니다.

## 구조
```
config/     대상 기업(업종 4분류) · 회계 키워드 · 리포트 소스
